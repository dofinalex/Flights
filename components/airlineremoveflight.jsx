...
import { deleteAirlineFlights, getAirlineFlights } from '../services/airlineServices';

class AirlineRemoveFlights extends Component {
    state={
       flight_lst:[],
       flight_lst1:[],
       pageSize:20,
    ...
    async componentDidMount(){
      try {
        ...
        const {data:flights}=await getAirlineFlights()
        console.log(flights)
        await  this.setState({
            flight_lst:[...flights]
          })        
      ...
      handlePageChange =page=>{
       ...
       handleDelete=async (flight)=>{
        const flights=await this.state.flight_lst.filter(c=>c.id !==flight.id)
        await this.setState({flight_lst:flights})
        console.log(flight.id)
        await deleteAirlineFlights(flight.id)
    ...
    handleFilter=async ()=>{
      ...
      await this.setState({flight_lst1:[...fl]}) 
    }
           ...
           <table className="table">
            ...
            <tbody>
                ...
                return(
                 ...
                 <td><button onClick={()=>this.handleDelete(flight)} className="btn btn-danger btn-sm">delete</button></td>
                </tr> 
                )  
                })}
...
export default AirlineRemoveFlights
